...
const AdminLoginView: React.FC<AdminLoginViewProps> = ({ onSuccess, onCancel, mode = 'login' }) => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user && user.email === 'janiceleroy@gmail.com') {
        onSuccess();
      }
    });
    return () => unsubscribe();
        ...
        <div className="space-y-6">
          {mode === 'login' ? (
            <button 
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-4 bg-white border border-gray-200 py-4 px-6 hover:bg-gray-50 transition-all group shadow-sm"
            >
